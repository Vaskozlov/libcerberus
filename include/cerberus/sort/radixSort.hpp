...
namespace cerb {
    ...
    auto RadixSort(T *data, size_t size) -> void {
        ...
        static_assert(sizeof(T) == sizeof(u8) || sizeof(T) == sizeof(u16) ||
                      sizeof(T) == sizeof(u32) || sizeof(T) == sizeof(u64));

        static_assert(std::is_trivially_copyable<T>::value);

        unsigned count[256];
        auto converted_data = reinterpret_cast<ByteMask<T> *>(data);
        auto output         = reinterpret_cast<ByteMask<T> *>(operator new(
            size * sizeof(ByteMask<T>)));

        for (unsigned shift = 0u, s = 0u; shift < sizeof(T); shift++, s += 8) {
            memset(count, 0u, sizeof(count) / sizeof(count[0]));
            ...
            swap(converted_data, output);
        }

        if (reinterpret_cast<ByteMask<T> *>(data) == output) {
            std::swap(converted_data, output);

            memcpy(converted_data, output, size);
        }

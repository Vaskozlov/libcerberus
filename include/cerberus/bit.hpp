...
template<typename T> constexpr auto isPowerOf2(T value) -> bool
{
 ...
 */
template<typename T> union CERBLIB_TRIVIAL ByteMask {
  T value;
 ...
 */
template<typename TO, typename FROM> [[nodiscard]] constexpr auto bit_cast(const FROM &x) noexcept -> TO
{
  static_assert(sizeof(TO) == sizeof(FROM) && std::is_trivially_copyable_v<FROM> && std::is_trivially_copyable_v<TO>);
...
template<typename T> class CERBLIB_TRIVIAL BitPattern
{
  static_assert(std::is_integral_v<T>);
...
public:
  constexpr friend auto operator==(const BitPattern<T> &pattern, const T &value) -> bool { return (value & pattern.m_mask) == pattern.m_expected; }

  constexpr friend auto operator==(const T &value, const BitPattern<T> &pattern) -> bool { return operator==(pattern, value); }

  constexpr friend auto operator!=(const BitPattern<T> &pattern, const T &value) -> bool { return !operator==(pattern, value); }

  constexpr friend auto operator!=(const T &value, const BitPattern<T> &pattern) -> bool { return !operator==(pattern, value); }

public:
  template<unsigned long SIZE> consteval explicit BitPattern(const char (&input)[SIZE])
  {

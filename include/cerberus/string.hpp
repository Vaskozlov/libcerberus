...
namespace cerb {
    ...
    constexpr auto memset(void *__restrict ptr, T value, size_t times) -> void {
        if (x86_64 && !std::is_constant_evaluated() &&
    ...
    constexpr auto memcpy(T *__restrict dest, const T *__restrict src, size_t times)
        -> void {
        if (x86_64 && sizeof(T) <= sizeof(uintmax_t) &&
            !std::is_constant_evaluated() &&
            (std::is_integral_v<T> || std::is_floating_point_v<T> ||
             std::is_default_constructible_v<T>)) {
            PRIVATE::memcpy(dest, src, times);
        } else {
            T *dest_copy           = dest;
    ...
    template<typename T> [[nodiscard]] consteval auto str2Uint(const char *str)
        -> T {
